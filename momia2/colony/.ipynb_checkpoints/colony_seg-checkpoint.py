...
class Colony:
    ...
    def measure_growth(self,edge_range=5):
        for l in np.unique(self.labels)[1:]:
            self.colony_growths[l] = np.array([(x==l).sum() for x in self.labels])
            self.is_touching_edge[l] = np.array([touching_edge(x,l,edge_width=edge_range) for x in self.labels])
...
def filter_by_growth(growth,min_dgrowth=100):
    accepted = False
    with np.errstate(divide='ignore',invalid='ignore'):
        ...
        gradient = g[1:]-g[:-1]
        if len(filtered_rel_dgrowth)>=2:
            if ((g.max()-g.min())>min_dgrowth):
                if (filtered_rel_dgrowth.max()<25) and (filtered_rel_dgrowth.min()>-25):
                    accepted=True
    return accepted