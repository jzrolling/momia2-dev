...
def _refine_trajectories(tracker,
    ...
    for i, p in enumerate(tracker.frames):
        p.labeled_mask = new_labels[i]
        h, w = p.labeled_mask.shape
        p.regionprops = None
            ...
            if v1 < 0.6 * v2:  # there should be a division event
                ...
                if turn_point >= 0:
                    if counts[:(turn_point + 1)].mean() < 1.5:
                        ...
                        for j in np.where((convolved_counts[(turn_point + 1):] <= 2) & (counts[(turn_point + 1):] == 1))[0]:
...
def transition_matrix(frames,t,
    ...
    transMatrix[:,:,6] = 1-(2*np.abs(area1[:,np.newaxis]-area2[np.newaxis])/(area1[:,np.newaxis]+area2[np.newaxis]))
